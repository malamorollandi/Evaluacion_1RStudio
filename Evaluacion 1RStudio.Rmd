...
Pantheria.Datos2 <- Pantheria.Datos1 %>%  group_by(Orden) %>%  summarise(Media.Tamaño.Camada = mean(Tamaño_Camada), Media.Camada.Año = mean(Camada_año), Media.Tiempo.Gestación = mean(Tiempo_Gestacion), Media.Densidad.Poblacional = mean(Densidad_Poblacion))
Pantheria.Datos3 <- Pantheria.Datos2 %>% filter(Media.Densidad.Poblacional > 500)
...
ggplot(Pantheria.Datos3, aes(x = Orden)) + 
  geom_col(aes(y = Media.Densidad.Poblacional),  fill="light blue", alpha =1.0) +
